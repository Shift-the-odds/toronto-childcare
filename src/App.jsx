        ...
        <header className="bg-white p-6 md:p-8 shadow-sm border border-gray-300">
          <div className="flex flex-col gap-4">
            <div>
              ...
              {/* Full-width balanced description block */}
              <div className="text-gray-700 text-sm md:text-base leading-relaxed space-y-4 pt-2">
                <p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeTab === 'scatter' && (
              <div className="bg-white border border-gray-300 p-6">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h2 className="text-xl text-black mb-2" style={{ fontFamily: serifFont }}>
                  ...
                  <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto min-w-[620px] bg-gray-50">
                    ...
                    {[0, 15, 30, 45, 60].map(tick => (
                      <g key={`y-${tick}`}>
                        ...
                        <text x={padding - 12} y={getY(tick) + 4} textAnchor="end" fontSize="10" fill="#6b7280">{tick}</text>
                      </g>
                    ))}
                    {[50000, 100000, 150000, 200000].map(tick => (
                      <g key={`x-${tick}`}>
                        ...
                        <text x={getX(tick)} y={height - padding + 22} textAnchor="middle" fontSize="10" fill="#6b7280">${tick / 1000}k</text>
                      </g>
                    ))}
                    ...
                    <line 
                      ...
                      strokeDasharray="3 3" 
                    />
                    <text x={getX(CITY_AVERAGES.income) + 6} y={padding + 15} fontSize="9" fill="#4b5563" className="font-semibold">City Avg Income (${CITY_AVERAGES.income/1000}k)</text>
                    ...
                    <line 
                      ...
                      strokeDasharray="3 3" 
                    />
                    <text x={width - padding - 150} y={getY(CITY_AVERAGES.spacesPer100) - 6} fontSize="9" fill="#4b5563" className="font-semibold">City Avg Coverage ({CITY_AVERAGES.spacesPer100}/100)</text>
                    ...
                    {/* Axis Labels */}
                    <text x={width / 2} y={height - 12} textAnchor="middle" fontSize="11" fill="#111827" style={{ fontFamily: serifFont }}>Median Household Income ($)</text>
                    <text x={18} y={height / 2} transform={`rotate(-90 18 ${height/2})`} textAnchor="middle" fontSize="11" fill="#111827" style={{ fontFamily: serifFont }}>Licensed Spaces per 100 Children (Coverage Rate)</text>

                    {/* Data Points */}
                    {filteredData.map((node) => {
                      const cx = getX(node.income);
                      const cy = getY(node.spacesPer100);
                      const r = Math.sqrt(node.totalChildren) * 0.35; // Scale radius
                ...
                {/* Plot Legends */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 pt-4 border-t border-gray-300 text-xs text-gray-700">
              ...
              <div className="bg-white border border-gray-300 p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-50 p-5 border border-gray-300">
                    <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Target Selection</span>
                    <h3 className="text-lg text-black mt-2" style={{ fontFamily: serifFont }}>{activeNeighbourhood.name}</h3>
                    <p className="text-sm text-gray-600 mt-2">Current base space capacity of {INITIAL_NEIGHBOURHOOD_DATA.find(n => n.id === activeNeighbourhood.id)?.capacity} spaces.</p>
                  ...
                  <div className="bg-white p-5 border border-gray-300 flex flex-col justify-between">
                    <div>
                      <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">Simulated Impact Summary</span>
                    ...
                    {simulatedSpaces[activeNeighbourhood.id] > 0 && (
                      <div className="text-sm text-teal-800 bg-teal-50 p-2.5 border border-teal-200 font-medium mt-4">
                ...
                <div className="pt-4">
                  <h4 className="text-xs font-semibold text-rose-700 uppercase tracking-wider mb-4 flex items-center gap-1.5">
                    ...
                    {processedData
                      ...
                      .map(item => (
                        ...
                        >
                          ...
                          <div className="text-sm font-semibold text-black truncate mb-2 pl-2">{item.name}</div>
                          <div className="flex justify-between items-center text-xs text-gray-600 pl-2">
            ...
            {activeTab === 'table' && (
              <div className="bg-white border border-gray-300 p-6 overflow-hidden">
                <div className="mb-6 border-b border-gray-300 pb-4">
                  <h2 className="text-xl text-black" style={{ fontFamily: serifFont }}>Neighbourhood Comparative Matrix</h2>
                </div>

                <div className="overflow-x-auto border border-gray-300">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-100 text-gray-700 text-xs font-semibold uppercase border-b border-gray-300">
            ...
            <div className={`bg-white border border-gray-300 border-l-4 p-6 transition-all duration-300 ${activeNeighbourhood.spacesPer100 < 15 ? 'border-l-red-600 bg-red-50/20' : 'border-l-black'}`}>
              <h3 className="text-lg text-black mb-4 font-semibold" style={{ fontFamily: serifFont }}>
          ...
          {/* Right Sidebar: Neighbourhood Details */}
          <div className="space-y-6">
            <div className="bg-white border border-gray-300 p-6 h-full flex flex-col justify-between">
              <div>
                <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-300">
                  <h2 className="text-lg text-black font-semibold" style={{ fontFamily: serifFont }}>
                ...
                {activeNeighbourhood ? (
                  <div className="space-y-8">
                    <div>
                      <div className="text-xs text-gray-500 font-semibold tracking-wider uppercase mb-1">{activeNeighbourhood.region} Sector</div>
                      <div className="text-xl text-black font-bold" style={{ fontFamily: serifFont }}>{activeNeighbourhood.name}</div>
                    </div>

                    {/* Regional Benchmarks with functional status coloring */}
                    <div className="space-y-5">
                      <div>
                        ...
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-sm font-semibold text-black">${activeNeighbourhood.income.toLocaleString()}</span>
                          <span className="text-[10px] text-gray-500 font-medium">City Avg: ${CITY_AVERAGES.income.toLocaleString()}</span>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="flex justify-between items-center mt-2">
                          <span className="text-sm font-semibold text-black">{activeNeighbourhood.singleParentPct}%</span>
                          <span className="text-[10px] text-gray-500 font-medium">City Avg: {CITY_AVERAGES.singleParentPct}%</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 p-5 border border-gray-300">
                      ...
                      <div className="space-y-3 text-sm text-black">
                        ...
                        <div className="pt-2 flex justify-between items-end">
                          <span className="font-semibold text-xs text-gray-600">Coverage Index:</span>
                          <span className={`text-xl font-bold ${activeNeighbourhood.spacesPer100 < 15 ? 'text-red-600' : 'text-teal-700'}`}>
                            {activeNeighbourhood.spacesPer100} <span className="text-xs text-gray-500 font-normal">spaces / 100 kids</span>
                          </span>
                        </div>
                      </div>
                    </div>

                    {/* Gender Impact Advisory */}
                    <div className={`p-5 border text-xs space-y-3 bg-white ${activeNeighbourhood.spacesPer100 < 15 ? 'border-red-300' : 'border-gray-300'}`}>
